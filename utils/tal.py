...
class TaskNearestAssigner(nn.Module):
    ...
    def get_targets(self, gt_labels, gt_cxy, gt_wh, target_gt_idx, fg_mask):
        batch_ind = torch.arange(end=self.bs, dtype=torch.int64, device=gt_labels.device)[..., None]
        target_gt_idx = target_gt_idx + batch_ind * self.n_max_boxes  # (b, h*w)
        target_labels = gt_labels.long().flatten()[target_gt_idx]  # (b, h*w)
        ...
        target_confs = torch.zeros((self.bs, self.ng, 1),
                                   dtype=torch.float,
                                   device=self.device)
    ...
    def forward(self, anc_whs, grids, gt_labels, gt_cxy, gt_wh, strides, mask_gt):
        ...
        for i in range(self.nl):
            ...
            self.get_targets(gt_labels, gt_cxy / stride, gt_wh / stride, target_gt_idx, fg_mask)
