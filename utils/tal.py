...
class TaskNearestAssigner(nn.Module):
    ...
    def get_targets(self, gt_labels, gt_cxy, gt_wh, bbox_deltas, target_gt_idx, mask_pos, fg_mask, ng):
        ...
        target_txy[fg_mask.bool()] = target_cxys[fg_mask.bool()] - (
                target_cxys[fg_mask.bool()] + bbox_deltas[mask_pos.bool()]
        ).int()
    ...
    def get_pos_mask(self, grid, gt_cxy, mask_gt):
        distance_deltas, bbox_deltas = self.get_box_metrics(grid, gt_cxy)
        ...
        bbox_deltas = bbox_deltas.view(self.bs, 1, self.n_max_boxes, -1, 2).expand(-1, self.na, -1, -1, -1)

        return mask_pos, bbox_deltas, distance_metric

    def get_box_metrics(self, grid, gt_cxy):
        n_anchors = grid.shape[0]
        gt_cxy = gt_cxy.view(-1, 1, 2)
        distance_deltas = (-grid[None] + gt_cxy + 0.5).view(self.bs, self.n_max_boxes, n_anchors, -1)
    ...
    def forward(self, anc_whs, grids, gt_labels, gt_cxy, gt_wh, strides, mask_gt):
        ...
        for i in range(self.nl):
            ...
            stride = strides[i]
            ng = grid.shape[0]
            mask_pos, bbox_deltas, distance_metric = self.get_pos_mask(grid, gt_cxy / stride, mask_gt)
