...
class TaskNearestAssigner(nn.Module):
    ...
    def get_pos_mask(self, grid, gt_cxy, strides, mask_gt):
        distance_deltas = self.get_box_metrics(grid, gt_cxy, strides)
    ...
    def get_box_metrics(self, grid, gt_cxy, strides):
        n_anchors = grid.shape[0]
        gt_cxy = gt_cxy.view(-1, 1, 2)
        distance_deltas = ((grid + strides / 2)[None] - gt_cxy).view(self.bs, self.n_max_boxes, n_anchors, -1)
    ...
    def forward(self, anc_whs, grids, gt_labels, gt_cxy, gt_wh, strides, mask_gt):
        ...
        mask_pos, distance_metric = self.get_pos_mask(grids, gt_cxy, strides, mask_gt)
        ...
        target_scores, target_txys, target_whs = self.get_targets(gt_labels,
                                                                  ...
                                                                  target_gt_idx,
                                                                  ng)
        anc_whs = anc_whs.view(1, 1, -1).expand(self.bs, ng, -1)
        r = target_whs / anc_whs
        mask_anc = torch.max(r, 1 / r).max(-1)[0] < self.anchor_t
        fg_mask = fg_mask * mask_anc

        return target_scores, target_scores, target_whs, anc_whs, fg_mask
