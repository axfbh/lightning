...
class TaskNearestAssigner(nn.Module):
    ...
    def get_targets(self, gt_labels, gt_cxy, gt_wh, grid, target_gt_idx, strides, ng):
        batch_ind = torch.arange(end=self.bs, dtype=torch.int64, device=gt_labels.device)[..., None]
        target_gt_idx = target_gt_idx + batch_ind * self.n_max_boxes  # (b, h*w)
    ...
    def get_pos_mask(self, grids, gt_cxys, strides, ngs, mask_gt):
        distance_deltas = self.get_box_metrics(grids, gt_cxys, strides)

        distance_metric = distance_deltas.abs().sum(-1)

        mask_topk = self.select_topk_candidates(distance_metric, ngs, largest=False)
    ...
    def select_topk_candidates(self, metrics, ngs, largest=True):
        # 每个 bbox 选取网格内 topk 个正样本
        topk_idxs = []
        i = 0
        for ng in ngs:
            _, ids = torch.topk(metrics[..., i:i + ng], self.topk, dim=-1, largest=largest)
            topk_idxs.append(ids + i)
            i += ng
        topk_idxs = torch.cat(topk_idxs, -1)
        count_tensor = torch.zeros(metrics.shape, dtype=torch.int8, device=metrics.device)
        count_tensor.scatter_(-1, topk_idxs, 1)
    ...
    @torch.no_grad()
    def forward(self, anc_wh, grids, gt_labels, gt_cxys, gt_whs, strides, ngs, mask_gt):
