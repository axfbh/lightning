...
class PaddingImage(DualTransform):
    ...
    def apply(
            self, img: np.ndarray, color=(114, 114, 114), pad_l=0, pad_t=0, pad_r=0, pad_b=0, **params: Any
    ) -> np.ndarray:
    ...
    def apply_to_bbox(self, bbox, pad_l=0, pad_t=0, pad_r=0, pad_b=0, **params) -> Tuple:
        return tuple([v1 + v2 for v1, v2 in zip(bbox, [pad_l, pad_t, pad_r, pad_b])])

    def apply_to_mask(self, mask, pad_l=0, pad_t=0, pad_r=0, pad_b=0, **params) -> np.ndarray:
